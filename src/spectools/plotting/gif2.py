...
def main():#kwargs de orbit o archivo kepler, espectros a mixear, factor d dilucion, cantidad de imagenes, regiones a plotear
    #para constructor de kepler.out
 #   columns = ['phase','VRA','VRB','E']
 #   data = pd.read_csv('kepler_song.out',skiprows=1,names=columns,sep=' ')
    #abir espectros y cortarlos a longitudes de onda < 7000AA
    ...
    orb = Orbit.from_linspace(100, e=.734628,omega=126.314, K1= 108.311,K2=-192.327,gamma = 0.)
...
#        rgbs = ["#{:02x}0000".format(ri) for ri in r]
        ...
        rgbs = ["#{:02x}00{:02x}".format(ri,bi) for ri,bi in zip(r,b)]
        return rgbs
